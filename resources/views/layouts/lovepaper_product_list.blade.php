...
<img class="lazyautosizes lazyload blur-up-big" src="/images/23-59228e2e9b14e9e72716070904534690-240-0.png" data-srcset="/images/23-59228e2e9b14e9e72716070904534690-480-0.png 480w, /images/23-59228e2e9b14e9e72716070904534690-640-0.png 640w, /images/23-59228e2e9b14e9e72716070904534690-1024-1024.png 1024w, /images/23-59228e2e9b14e9e72716070904534690-1920-1920.png 1920w" data-sizes="auto" alt="Banner de la categoría LOVE PAPER CASE" />
</div>
</div>
<div class="js-category-breadcrumbs row-fluid container-wide categoria container-xs m-bottom m-none-xs m-bottom-quarter-xs">
<div class="span8 left">
<div class="breadcrumb m-bottom-half m-top-half" data-store="page-title">
<a class="h6 font-body-xs crumb" href="/" title="LOVE PAPER">Inicio</a>
<span class="breadcrumb-divider">/</span>

@if($category=='' && $subcategory=='')
<h1 class="breadcrumb-crumb active">LOVE PAPER CASE</h1>
@endif

@if($category!='' && $subcategory=='')
<a class="h6 font-body-xs crumb" href="/productos/" title="LOVE PAPER">LOVE PAPER CASE</a> / <h1 class="breadcrumb-crumb active">{{$categoryName}}</h1> 
@endif

@if($category!='' && $subcategory!='')
<a class="h6 font-body-xs crumb" href="/productos/" title="LOVE PAPER">LOVE PAPER CASE</a> / 
<a class="h6 font-body-xs crumb" href="/productos/{{$category}}" title="LOVE PAPER">{{$categoryName}}</a>  / <h1 class="breadcrumb-crumb active">{{$subcategoryName}}</h1> 
@endif
...
<select id="sort" class="sort-by full-width-xs filter-order">
...
<h4 class="m-bottom p-bottom-half">LOVE PAPER CASE</h4>

<ul class="list-unstyled ">

@if($category=='' && $subcategory=='')
    @foreach($categories as $itemCategory)
        <li data-item="1" class="m-bottom-quarter"><a href="/productos/{{$itemCategory->slug}}/" title="{{$itemCategory->name}}">{{$itemCategory->name}}</a></li>
    @endforeach
@endif
...
@if($category!='' && $subcategory=='')
    @foreach($categories as $itemCategory)
        <li data-item="1" class="m-bottom-quarter"><a href="/productos/{{$category}}/{{$itemCategory->slug}}/" title="{{$itemCategory->name}}">{{$itemCategory->name}}</a></li>
    @endforeach
@endif
...
<div class="span10">
<div class="js-product-table pull-left row-fluid m-bottom m-top-xs" data-store="category-grid-6724700">
...
<!-- producto -->
<div class="span3 item-container m-bottom-half" data-store="product-item-65098911">
<div class="js-item-product item m-top-none-xs" data-product-type="list" data-product-id="65098911" data-component="product-list-item" data-component-value="65098911">
<div class="js-item-image-container  item-image-container">
<div style="padding-bottom: 100%;" class="js-item-image-padding p-relative" data-store="product-item-image-65098911">
<a href="{{ route('product.show',['id'=>$product->product_id,'slug'=>$product->product_slug])}}" title="{{ $product->product_name }}" aria-label="{{ $product->product_name }}">

<picture>
<source class="js-item-image-source" media="(max-width: 767px)" data-srcset="{{ asset('/storage/'.$product->product_image) }} 240w,{{ asset('/storage/'.$product->product_image) }} 320w">
<source class="js-item-image-source" media="(min-width: 768px)" data-srcset="/{{ asset('/storage/'.$product->product_image) }} 240w, {{ asset('/storage/'.$product->product_image) }} 320w">
<img src="{{ asset('/storage/'.$product->product_image) }}" data-srcset="{{ asset('/storage/'.$product->product_image) }} 240w, {{ asset('/storage/'.$product->product_image) }} 320w" alt="{{ $product->product_name }}" data-expand="-10" class="js-item-image js-item-image-primary lazyload item-image img-absolute img-absolute-centered fade-in " width="2481" height="2481">
...
<div class="js-offer-label item-label item-label-offer product-label label-primary label label-accent" data-store="product-item-offer-label" style="display: none;">
<span class="js-offer-percentage">{{$i}}</span>
% OFFF
</div>
</div>
</div>
<div class="item-info-container" data-store="product-item-info-65098911">
<a href="{{ route('product.show',['id'=>$product->product_id,'slug'=>$product->product_slug])}}" title="{{ $product->product_name }}" class="js-item-name item-name" aria-label="{{ $product->product_name }}" data-store="product-item-name-65098911">
...
<div class="product-grid-labels" style="display: none;">
    ...
    @if($product->total_stock>0)
    <div class="js-stock-label item-label product-label label-light font-small" style="display:;">STOCK: {{$product->total_stock}}</div>
    @endif
</div>
</div>
...
<h5>{{ $products->links('products.pagination') }}</h5>
...
@if($category=='' && $subcategory=='')
    @foreach($categories as $itemCategory)
        ...
        <li data-item="1" class="pill"><a href="/productos/{{$itemCategory->slug}}/" title="{{$itemCategory->name}}" class="pill-link">{{$itemCategory->name}}</a></li>
...
@if($category!='' && $subcategory=='')
    @foreach($categories as $itemCategory)
        <li data-item="1" class="pill"><a href="/productos/{{$category}}/{{$itemCategory->slug}}/" title="{{$itemCategory->name}}" class="pill-link">{{$itemCategory->name}}</a></li>
...
<span class="cart-total-price js-cart-subtotal pull-right text-right weight-strong" data-priceraw="0" data-component="cart.subtotal" data-component-value="0">$0,00</span>
</div>
...
window.onload = function(){
        ...
        if(urlParams.has('variants')){
            var variants = urlParams.get('variants');
            variants = variants.split(',');
            for(var v = 0; v < variants.length; v++){
                $('#variant_'+variants[v]).attr('checked','checked');
                $('#variant_mobile_'+variants[v]).attr('checked','checked');
            }  
        }

        if(urlParams.has('order')){
            $('.filter-order').val(urlParams.get('order'));
        }
        ...
        $('.filter-checkbox').click(function(){
            ...
            for(var i = 0; i< $('.filter-checkbox').length; i++){
                if($($('.filter-checkbox')[i]).prop('checked')){
                    variants = variants + $($('.filter-checkbox')[i]).attr('data-id') + ",";
                }
            }
            ...
            document.location.href = window.location.pathname+"?variants="+variants+order;
        });

        $('.filter-checkbox-mobile').click(function(){
            var variants = "";
            
            var urlParams = new URLSearchParams(window.location.search);

            for(var i = 0; i< $('.filter-checkbox-mobile').length; i++){
                if($($('.filter-checkbox-mobile')[i]).prop('checked')){
                    variants = variants + $($('.filter-checkbox-mobile')[i]).attr('data-id') + ",";
                }
            }
            variants = variants.slice(0, -1);
        ...
        $('.filter-order').change(function(){
            ...
            document.location.href = window.location.pathname+"?variants="+variants+"&order="+$(this).val();
        });
